...
# Modified 2026-06-03 15:06 KST (feature/version2): version2 4-feature protocol payload length.
# peak_usage_pressure(4) + climate_volatility_index(4) + avg_power(4) + month(4) = 16 bytes
PAYLOAD_LEN = 16
...
class Server:
    def __init__(
        ...
        self.dimension = dimension  # feature의 차원. feature/version2 protocol에서는 4
        self.index = index  # power value의 index. avg_power가 2번째이므로 feature/version2 protocol에서는 2
    ...
    # Modified 2026-06-03 15:06 KST (feature/version2): TCP recv can return partial data, so read exactly size bytes.
    ...
    # Modified 2026-06-03 15:06 KST (feature/version2): decode 16-byte payload for [peak_usage_pressure, climate_volatility_index, avg_power, month].
    def parse_data(self, buf, is_training):
        peak_usage_pressure = int.from_bytes(buf[0:4], byteorder="big", signed=True)
        climate_volatility_index = int.from_bytes(buf[4:8], byteorder="big", signed=True)
        avg_power = int.from_bytes(buf[8:12], byteorder="big", signed=True)
        month = int.from_bytes(buf[12:16], byteorder="big", signed=True)

        lst = [peak_usage_pressure, climate_volatility_index, avg_power, month]
        logging.info("[peak_usage_pressure_x100, climate_volatility_index, avg_power, month] = {}".format(lst))
    ...
    def handler(self, client):
        ...
        while True:
            ...
            if opcode == OPCODE_DATA:  # 잘 수행되었으면 OPCODE_DATA = 1을 받아야함.
                ...
                rbuf = self.recv_exact(
                    client, PAYLOAD_LEN
                )  # Modified 2026-06-03 15:06 KST (feature/version2): receive 16-byte payload for version2 protocol.
        ...
        while ntest > 0:
            ...
            if opcode == OPCODE_DATA:
                logging.info("[*] data report from the edge")
                rbuf = self.recv_exact(client, PAYLOAD_LEN)  # Modified 2026-06-03 15:06 KST (feature/version2): receive 16-byte testing payload.
                logging.debug("[*] received buf: {}".format(rbuf))
...
# Modified 2026-06-03 15:06 KST (feature/version2) 예시 : python3 server.py --algorithm lstm --dimension 4 --index 2 --ntrain 10 --ntest 7 --name version2_test --caddr 127.0.0.1 --cport 5556 --lport 6000 --log DEBUG
