...
// Modified 2026-06-03 15:06 KST (feature/version2): send payload using dlen from ProcessManager.
...
// dlen은 process_manager.cpp에서 processData()에서 만든 payload의 길이
{
  ...
  sock = this->sock;
  // Modified 2026-06-03 15:06 KST (feature/version2) payload: peak_usage_pressure(4) || climate_volatility_index(4) || avg_power(4) || month(4)
  ...
  // Modified 2026-06-03 15:06 KST (feature/version2): payload 길이는 processData()가 계산한 dlen을 그대로 사용한다.
  tbs = dlen;
  offset = 0;
  while (offset < tbs)
  {
