...
// sendData() is the Edge -> Control Server transmission point.
// Packet format:
//   OPCODE_DATA (1 byte) || payload generated by ProcessManager::processData()
// data points to the payload buffer, and dlen is the exact payload length.
// Because sendData() trusts dlen, changing the feature vector only requires
// processData() and server.py to agree on payload length and field order.
...
// receiveCommand() waits for the Control Server's flow-control command.
// The Edge sends one day of data, then calls this function before moving to
// the next day. The server may reply with:
//   OPCODE_WAIT: training is not ready yet; keep waiting.
//   OPCODE_DONE: server is ready; continue sending the next dataset.
//   OPCODE_QUIT: training/testing is complete; stop the Edge loop.
...
uint8_t NetworkManager::receiveCommand() // 서버로부터 명령을 받음.
{
  ...
  while (opcode == OPCODE_WAIT) // 서버가 WAIT을 보내면 계속 기다림. 서버가 DONE이나 QUIT을 보낼 때까지 반복
    // The command packet is exactly 1 byte, matching opcode.h.
    read(sock, &opcode, 1);
